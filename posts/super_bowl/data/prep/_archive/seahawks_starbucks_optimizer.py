"""
Seahawks Starbucks Gravity Optimizer

Finds creative stats showing the Seahawks perform better when Starbucks gravity is higher.
Uses the same optimization approach as super_bowl_optimizer.py but filters by Starbucks gravity zones.
"""
...
# Offensive Metrics (for when SEA is on offense)
OFFENSE_METRICS = {
    # Rushing Performance (key metric from coffee report)
...
def calculate_coffee_gravity(client):
    ...
    # Define Gravity Zones based on the coffee narrative report
    # From the report: Levi's Stadium at -5.80 is "Starbucks Death Zone" (< -4.5)
    # Gillette at +4.35 is "Dunkin Safe Zone" (> -4.5)
    # We'll create more granular zones:
    
    def classify_gravity(net_grav):
        if net_grav <= -7:
            return 'Extreme Starbucks Zone'
        elif net_grav <= -4.5:
            return 'Starbucks Death Zone'
        elif net_grav <= -1:
            return 'Mild Starbucks Zone'
        elif net_grav <= 1:
            return 'Neutral Zone'
        elif net_grav <= 3:
...
def load_defense_data(client, gravity_df):
    """Load SEA defense data with coffee gravity context"""
    print("Loading SEA Defense Data...")
    ...
    FROM `{BQ_PBP_TABLE}`
    WHERE season = 2025
        AND (posteam = 'SEA' OR defteam = 'SEA')
    """
    
    schedule_df = client.query(schedule_query).to_dataframe()
    
    # For defense, SEA is the defteam
    schedule_df = schedule_df[schedule_df['defteam'] == 'SEA']
...
def load_offense_data(client, gravity_df):
    """Load SEA offensive play-by-play data with coffee gravity"""
    print("Loading SEA Offense Data (Play-by-Play)...")
    
    # Get play-by-play data for SEA offense
    pbp_query = f"""
    SELECT
        pbp.week,
        pbp.home_team,
...
def build_filters():
    ...
    # Coffee Gravity Zones (the key filters!)
    filters['Coffee Gravity'] = {
        'Extreme Starbucks Zone': "gravity_zone = 'Extreme Starbucks Zone'",
        'Starbucks Death Zone': "gravity_zone = 'Starbucks Death Zone'",
        'High Starbucks (Net < -4)': "net_gravity <= -4",
        'Any Starbucks Zone (Net < 0)': "net_gravity < 0",
        'Neutral Zone': "gravity_zone = 'Neutral Zone'",
        'Dunkin Territory (Net > 0)': "net_gravity > 0",
    }
...
def generate_report(perms, target, type_, gravity_df):
    ...
    if type_ == 'defense':
        filename = os.path.join(output_dir, "seahawks_defense_starbucks.md")
    else:
        filename = os.path.join(output_dir, "seahawks_offense_starbucks.md")
    ...
    with open(filename, 'w') as f:
        f.write(f"# Seahawks {type_.upper()} Performance by Starbucks Gravity\n\n")
        f.write(f"**Analysis Period:** 2025 Season\n\n")
        f.write(f"This analysis shows how the Seattle Seahawks {'defense' if type_ == 'defense' else 'offense'} performs under different **Coffee Gravity** conditions.\n\n")
        ...
        f.write("\n\n")
        f.write("**Key Insight:** Seattle benefits from high Starbucks gravity, Patriots from high Dunkin' gravity.\n\n")
        f.write("---\n\n")
        ...
        for rank_type, p_list in perms_by_type.items():
            ...
            for i, perm in enumerate(p_list[:15]):  # Top 15
                ...
                # Add narrative
                is_starbucks_condition = any('Starbucks' in fd for fd in filter_descs)
                if is_starbucks_condition and rank_type == 'Best':
                    f.write(f"✅ **NARRATIVE WIN:** Seahawks excel in Starbucks territory!\n\n")
                elif is_starbucks_condition and rank_type == 'Worst':
                    f.write(f"⚠️ This is a worst-case scenario in Starbucks land.\n\n")
...
def main():
    ...
    print("\n🏈 Lumen Field (SEA) Gravity:")
    sea_gravity = gravity_df[gravity_df['team_abbr'] == 'SEA']
    print(sea_gravity)
    
    filters = build_filters()
    
    # Analysis 1: SEA Defense in Starbucks Zones
    print("\n\n🔍 ANALYZING: SEA Defense Performance by Starbucks Gravity")
    print("=" * 70)
    def_df = load_defense_data(client, gravity_df)
    
    # Force filter: any starbucks condition
    starbucks_filter = ('Coffee Gravity', 'Any Starbucks Zone (Net < 0)', 
                        filters['Coffee Gravity']['Any Starbucks Zone (Net < 0)'])
    
    def_perms = run_optimization(def_df, 'SEA', 'defense', filters, 
                                 forced_filter=starbucks_filter)
    
    print(f"\n✅ Found {len(def_perms)} defensive stuperlatives")
    if def_perms:
        generate_report(def_perms, 'SEA', 'defense', gravity_df)
    
    # Analysis 2: SEA Offense in Starbucks Zones  
    print("\n\n🔍 ANALYZING: SEA Offense Performance by Starbucks Gravity")
    print("=" * 70)
    off_df = load_offense_data(client, gravity_df)
    
    off_perms = run_optimization(off_df, 'SEA', 'offense', filters,
                                 forced_filter=starbucks_filter)
    
    print(f"\n✅ Found {len(off_perms)} offensive stuperlatives")
    if off_perms:
        generate_report(off_perms, 'SEA', 'offense', gravity_df)
